...
sub decode_message {
    my ( $frame ) = @_;
    my ( $msg_id, $protocol_id, $length, $unit_id, $type ) = unpack("nnnCC", $frame);
    my $crc = unpack("n", bytes::substr($frame, -2));
    my $frame_part = bytes::substr($frame, 0, -2);
    my $data = _xor(bytes::substr( $frame_part, 8));

    print("decode_message\tmessage CRC\t", $crc, "\t calculated CRC\t", _crc($frame_part),"\n") if $debug > 3;
...
sub process_message {
    ...
    print("process_message", "\t", "type", "\t", sprintf("0x%02X", $message->{type}), "\n") if $debug > 2;
    print("process_message", "\t", "data", "\n", Hexify(\$message->{data})) if $debug > 2;

    # PING.
    if ( $message->{type} == MSG_TYPE_PING ) {
      my $request = decode_ping_request($message); 
      print( $ts, "\t", "== received PING from ", $request->{serial}, ", sending reply ==\n\n" ) if $debug;
      return create_reply(MSG_TYPE_PING, $message, pack("a10", $request->{serial}));
    }
    ...
    if ( $message->{type} == MSG_TYPE_DATA) {
      ...
      print( $ts, "\t", "== received DATA from ", $request->{serial}, ", sending reply ==\n\n" ) if $debug;
...
sub decode_query_request {
    my ( $message ) = @_;
    my ( $serial, $config_id, $config_value ) = unpack("a10 x21 C x2 a*", $message->{data});
...
sub decode_data_request {
    ...
    my ( $serial, $inverter ) = unpack("a10 x20 a10", $message->{data});
    my ( $year, $month, $day ) = unpack("CCC", substr($message->{data}, 60));
    my ( $hour, $min, $sec ) = unpack("CCC", substr($message->{data}, 63));  
    my ( $Ppv ) = unpack("N", substr($message->{data}, 73));
    my ( $Vpv1, $Ipv1, $Ppv1 ) = unpack("nnN", substr($message->{data}, 77));
    my ( $Pac, $Fac ) = unpack("Nn", substr($message->{data}, 117));
    my ( $Vac1, $Iac1, $Pac1 ) = unpack("nnN", substr($message->{data}, 123));
